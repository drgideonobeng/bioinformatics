...
message("--- Step 5: Functional Gene Annotation ---")

# 1. Load the significant results from Script 07
results_file <- path("results", "DE_results_Treated_vs_Control.csv")
if(!file_exists(results_file)) stop("Error: Cannot find the DE results CSV.")
...
res_annotated <- res_tidy %>%
  ...
  # Reorder columns so the biological names are right next to the IDs
  relocate(symbol, description, .after = gene)

# 3. Save the final, biologist-friendly CSV
out_file <- path("results", "DE_results_Annotated.csv")
...
# Save the Top 50 to its own file
top50_file <- path("results", "DE_results_Top50_Annotated.csv")
write_csv(top_50_genes, top50_file)
message("=> Saved Top 50 snapshot to: ", top50_file)

# Print a quick terminal preview of the top 5 (just the most important columns)
message("\n--- Top 10 DE Genes ---")
top_50_genes %>% 
