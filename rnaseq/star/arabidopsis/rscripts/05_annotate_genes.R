...
for (results_file in csv_files) {
    ...
    res_annotated <- res_tidy %>%
      mutate(
        symbol = mapIds(org.At.tair.db, keys = GeneID, column = "SYMBOL", keytype = "TAIR", multiVals = "first"),
        description = mapIds(org.At.tair.db, keys = GeneID, column = "GENENAME", keytype = "TAIR", multiVals = "first")
      ) %>%
