...
# Strip version numbers from the Ensembl IDs for the database search
significant_gene_ids <- sub("\\..*", "", sig_degs$GeneID)

message("Running GO Enrichment...")
go_results <- enrichGO(gene = significant_gene_ids, OrgDb = org.Hs.eg.db, keyType = "ENSEMBLTRANS", ont = "BP", pAdjustMethod = "BH", pvalueCutoff = 0.05, qvalueCutoff = 0.05)
